...
export class WebCrawler {
  private static readonly DEFAULT_USER_AGENT = 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36';
  ...
  static async crawl(url: string, options: CrawlerOptions = {}): Promise<CrawledContent> {
  ...
  private static async crawlStatic(url: string, options: Required<CrawlerOptions>): Promise<CrawledContent> {
    ...
    if (!content.title || content.title === 'Untitled' || content.title.length <= 3) {
  ...
  private static async crawlDynamic(url: string, options: Required<CrawlerOptions>): Promise<CrawledContent> {
  ...
  private static extractContentFromHtml(html: string, url: string): CrawledContent {
